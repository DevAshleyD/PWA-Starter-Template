define(["exports","./my-app.js"],function(_exports,_myApp){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.createSelectorCreator=createSelectorCreator;_exports.createStructuredSelector=createStructuredSelector;_exports.defaultMemoize=defaultMemoize;_exports.removeFromCart=_exports.getAllProducts=_exports.createSelector=_exports.checkout=_exports.cartTotalSelector=_exports.cartQuantitySelector=_exports.cartItemsSelector=_exports.addToCartUnsafe=_exports.addToCart=_exports.REMOVE_FROM_CART=_exports.GET_PRODUCTS=_exports.CHECKOUT_SUCCESS=_exports.CHECKOUT_FAILURE=_exports.ADD_TO_CART=_exports.$shopDefault=_exports.$shop$1=_exports.$shop=_exports.$index=void 0;function ownKeys(object,enumerableOnly){var keys=Object.keys(object);if(Object.getOwnPropertySymbols){keys.push.apply(keys,Object.getOwnPropertySymbols(object))}if(enumerableOnly)keys=keys.filter(function(sym){return Object.getOwnPropertyDescriptor(object,sym).enumerable});return keys}function _objectSpread(target){for(var i=1,source;i<arguments.length;i++){source=null!=arguments[i]?arguments[i]:{};if(i%2){ownKeys(source,!0).forEach(function(key){babelHelpers.defineProperty(target,key,source[key])})}else if(Object.getOwnPropertyDescriptors){Object.defineProperties(target,Object.getOwnPropertyDescriptors(source))}else{ownKeys(source).forEach(function(key){Object.defineProperty(target,key,Object.getOwnPropertyDescriptor(source,key))})}}return target}function defaultEqualityCheck(a,b){return a===b}function areArgumentsShallowlyEqual(equalityCheck,prev,next){if(null===prev||null===next||prev.length!==next.length){return!1}// Do this in a for loop (and not a `forEach` or an `every`) so we can determine equality as fast as possible.
...
return memoizedResultFunc.apply(null,params)});selector.resultFunc=resultFunc;selector.dependencies=dependencies;selector.recomputations=function(){return recomputations};selector.resetRecomputations=function(){return recomputations=0};return selector}}var createSelector=createSelectorCreator(defaultMemoize);_exports.createSelector=createSelector;function createStructuredSelector(selectors){var selectorCreator=1<arguments.length&&arguments[1]!==void 0?arguments[1]:createSelector;if("object"!==typeof selectors){throw new Error("createStructuredSelector expects first argument to be an object "+("where each property is a selector, instead received a "+typeof selectors))}var objectKeys=Object.keys(selectors);return selectorCreator(objectKeys.map(function(key){return selectors[key]}),function(){for(var _len3=arguments.length,values=Array(_len3),_key3=0;_key3<_len3;_key3++){values[_key3]=arguments[_key3]}return values.reduce(function(composition,value,index){composition[objectKeys[index]]=value;return composition},{})})}var index={defaultMemoize:defaultMemoize,createSelectorCreator:createSelectorCreator,createSelector:createSelector,createStructuredSelector:createStructuredSelector};/**
   ...
   */_exports.$index=index;const GET_PRODUCTS="GET_PRODUCTS";_exports.GET_PRODUCTS=GET_PRODUCTS;const ADD_TO_CART="ADD_TO_CART";_exports.ADD_TO_CART=ADD_TO_CART;const REMOVE_FROM_CART="REMOVE_FROM_CART";_exports.REMOVE_FROM_CART=REMOVE_FROM_CART;const CHECKOUT_SUCCESS="CHECKOUT_SUCCESS";_exports.CHECKOUT_SUCCESS=CHECKOUT_SUCCESS;const CHECKOUT_FAILURE="CHECKOUT_FAILURE";_exports.CHECKOUT_FAILURE=CHECKOUT_FAILURE;const PRODUCT_LIST=[{id:1,title:"Cabot Creamery Extra Sharp Cheddar Cheese",price:10.99,inventory:2},{id:2,title:"Cowgirl Creamery Mt. Tam Cheese",price:29.99,inventory:10},{id:3,title:"Tillamook Medium Cheddar Cheese",price:8.99,inventory:5},{id:4,title:"Point Reyes Bay Blue Cheese",price:24.99,inventory:7},{id:5,title:"Shepherd's Halloumi Cheese",price:11.99,inventory:3}],getAllProducts=()=>dispatch=>{// Here you would normally get the data from the server. We're simulating
// that by dispatching an async action (that you would dispatch when you
// successfully got the data back).
// You could reformat the data in the right format as well.
const products=PRODUCT_LIST.reduce((obj,product)=>{obj[product.id]=product;return obj},{});dispatch({type:GET_PRODUCTS,products})};_exports.getAllProducts=getAllProducts;const checkout=()=>dispatch=>{// Here you could do things like credit card validation, etc.
// If that fails, dispatch CHECKOUT_FAILURE. We're simulating that
// by flipping a coin :)
const flip=Math.floor(2*Math.random());if(0===flip){dispatch({type:CHECKOUT_FAILURE})}else{dispatch({type:CHECKOUT_SUCCESS})}};_exports.checkout=checkout;const addToCart=productId=>(dispatch,getState)=>{const state=getState();// Just because the UI thinks you can add this to the cart
// doesn't mean it's in the inventory (user could've fixed it).
if(0<state.shop.products[productId].inventory){dispatch(addToCartUnsafe(productId))}};_exports.addToCart=addToCart;const removeFromCart=productId=>{return{type:REMOVE_FROM_CART,productId}};_exports.removeFromCart=removeFromCart;const addToCartUnsafe=productId=>{return{type:ADD_TO_CART,productId}};_exports.addToCartUnsafe=addToCartUnsafe;var shop={GET_PRODUCTS:GET_PRODUCTS,ADD_TO_CART:ADD_TO_CART,REMOVE_FROM_CART:REMOVE_FROM_CART,CHECKOUT_SUCCESS:CHECKOUT_SUCCESS,CHECKOUT_FAILURE:CHECKOUT_FAILURE,getAllProducts:getAllProducts,checkout:checkout,addToCart:addToCart,removeFromCart:removeFromCart,addToCartUnsafe:addToCartUnsafe};_exports.$shop=shop;const INITIAL_STATE={products:{},cart:{},error:""},shop$1=(state=INITIAL_STATE,action)=>{switch(action.type){case GET_PRODUCTS:return _objectSpread({},state,{products:action.products});case ADD_TO_CART:case REMOVE_FROM_CART:case CHECKOUT_SUCCESS:return _objectSpread({},state,{products:products(state.products,action),cart:cart(state.cart,action),error:""});case CHECKOUT_FAILURE:return _objectSpread({},state,{error:"Checkout failed. Please try again"});default:return state;}};_exports.$shopDefault=shop$1;// Slice reducer: it only reduces the bit of the state it's concerned about.
const products=(state,action)=>{switch(action.type){case ADD_TO_CART:case REMOVE_FROM_CART:const productId=action.productId;return _objectSpread({},state,{[productId]:product(state[productId],action)});default:return state;}},product=(state,action)=>{switch(action.type){case ADD_TO_CART:return _objectSpread({},state,{inventory:state.inventory-1});case REMOVE_FROM_CART:return _objectSpread({},state,{inventory:state.inventory+1});default:return state;}},cart=(state,action)=>{switch(action.type){case ADD_TO_CART:const addId=action.productId;return _objectSpread({},state,{[addId]:(state[addId]||0)+1});case REMOVE_FROM_CART:const removeId=action.productId,quantity=(state[removeId]||0)-1;if(0>=quantity){const newState=_objectSpread({},state);delete newState[removeId];return newState}else{return _objectSpread({},state,{[removeId]:quantity})}case CHECKOUT_SUCCESS:return{};default:return state;}},cartSelector=state=>state.shop.cart,productsSelector=state=>state.shop.products,cartItemsSelector=createSelector(cartSelector,productsSelector,(cart,products)=>{return Object.keys(cart).map(id=>{const item=products[id];return{id:item.id,title:item.title,amount:cart[id],price:item.price}})});_exports.cartItemsSelector=cartItemsSelector;// Return the total cost of the items in the cart
const cartTotalSelector=createSelector(cartSelector,productsSelector,(cart,products)=>{let total=0;Object.keys(cart).forEach(id=>{const item=products[id];total+=item.price*cart[id]});return Math.round(100*total)/100});// Return the number of items in the cart
_exports.cartTotalSelector=cartTotalSelector;const cartQuantitySelector=createSelector(cartSelector,cart=>{let num=0;Object.keys(cart).forEach(id=>{num+=cart[id]});return num});_exports.cartQuantitySelector=cartQuantitySelector;var shop$2={default:shop$1,cartItemsSelector:cartItemsSelector,cartTotalSelector:cartTotalSelector,cartQuantitySelector:cartQuantitySelector};_exports.$shop$1=shop$2;class ShopItem extends _myApp.LitElement{static get properties(){return{name:{type:String},amount:{type:String},price:{type:String}}}render(){return _myApp.html`
      ${this.name}:
    ...
    `}}window.customElements.define("shop-item",ShopItem);class ShopProducts extends(0,_myApp.connect)(_myApp.store)(_myApp.LitElement){static get properties(){return{_products:{type:Object}}}static get styles(){return[_myApp.ButtonSharedStyles,_myApp.css`
      ...
      ${Object.keys(this._products).map(key=>{const item=this._products[key];return _myApp.html`
        ...
        `})}
    `}firstUpdated(){_myApp.store.dispatch(getAllProducts())}_addButtonClicked(e){_myApp.store.dispatch(addToCart(e.currentTarget.dataset.index))}// This is called every time something is updated in the store.
stateChanged(state){this._products=state.shop.products}}window.customElements.define("shop-products",ShopProducts);class ShopCart extends(0,_myApp.connect)(_myApp.store)(_myApp.LitElement){static get properties(){return{_items:{type:Array},_total:{type:Number}}}static get styles(){return[_myApp.ButtonSharedStyles,_myApp.css`
        :host {
          display: block;
        }
      `]}render(){return _myApp.html`
    ...
    `}_removeButtonClicked(e){_myApp.store.dispatch(removeFromCart(e.currentTarget.dataset.index))}// This is called every time something is updated in the store.
stateChanged(state){this._items=cartItemsSelector(state);this._total=cartTotalSelector(state)}}window.customElements.define("shop-cart",ShopCart);_myApp.store.addReducers({shop:shop$1});// These are the elements needed by this element.
class MyView3 extends(0,_myApp.connect)(_myApp.store)(_myApp.PageViewElement){static get properties(){return{// This is the data from the store.
_quantity:{type:Number},_error:{type:String}}}static get styles(){return[_myApp.SharedStyles,_myApp.ButtonSharedStyles,_myApp.css`
        button {
    ...
    `}_checkoutButtonClicked(){_myApp.store.dispatch(checkout())}// This is called every time something is updated in the store.
stateChanged(state){this._quantity=cartQuantitySelector(state);this._error=state.shop.error}}window.customElements.define("my-view3",MyView3)});